"""Evaluating every position of a finished Ultimate Tic-Tac-Toe game.
...
worth noting is that a position is rebuilt by *replaying the moves*: the board a
player is confined to follows from the last move, so a prefix of the move list
is the only compact way to name a position here.
"""
...
from .uttt_game import N, X, UltimateBoard, move_to_str

C_PUCT = 1.6


def readout(search: Search, board: UltimateBoard) -> dict:
    """What the review keeps for one position."""
    # Everything is reported from X's point of view, so neither the graph nor
    # the numbers flip meaning every ply.
    first = 1.0 if board.to_move == X else -1.0
    ...
    return dict(
        ...
        best=best,
        best_label=move_to_str(best) if best is not None else None,
...
def terminal_readout(board: UltimateBoard) -> dict:
    """A finished position is a certainty, not a network estimate."""
    if board.winner == 0:
        value = 0.0  # every small board decided, no line among them: drawn
    else:
        value = 1.0 if board.winner == X else -1.0
...
def evaluate_position(task: tuple) -> dict:
    ...
    awkward to ship between processes than a list of ints -- and because the
    prefix is what determines the constraint the position is under.
    """
