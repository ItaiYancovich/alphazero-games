...
Same shape as the Hex registry, and deliberately the same ``kind`` strings --
``random``, ``rule``, ``minimax``, ``rollout``, ``policy``, ``az`` -- so the
rating keys, the GUI's opponent ids and the ladder scripts read identically for
both games.
"""
...
def evaluator(ckpt: str, rows: int = ROWS, cols: int = COLS, device: str = "cpu") -> BatchEvaluator:
    ...
    """
    key = (ckpt, rows, cols, device.lower())
...
def build_agent(s: AgentSpec, seed: int, rows: int = ROWS, cols: int = COLS) -> Agent:
    p = s.as_dict()
    if s.kind == "random":
    ...
    if s.kind == "minimax":
        return MinimaxAgent(time_budget=p.get("time_budget", 1.0),
                            max_depth=p.get("max_depth", 42), seed=seed, name=s.label)
    if s.kind == "rollout":
...
def default_field(ckpt: str) -> list[AgentSpec]:
    ...
    return [
        spec("random", "random"),
        spec("rule", "rule-based (windows + centre)", noise=0.05),
