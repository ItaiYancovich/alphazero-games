"""Connect Four's rating table and player profiles.

Same machinery as Hex (:mod:`alphazero_core.ratings`), separate files: a player
is a different strength at the two games and the bot fields have nothing in
common beyond both being anchored at ``random`` = 1000.

The one addition is draws, which Hex does not have: a drawn rated game scores
half a point, which is what :func:`record_result` takes here.
"""
...
RATINGS_PATH = ROOT / "runs" / "ratings_c4.json"
USERS_PATH = ROOT / "runs" / "users_c4.json"

STORE = RatingStore(RATINGS_PATH, USERS_PATH, game="c4")
...
def record_result(name: str, opponent_key: str, opponent_rating: float,
    ...
    """Apply one rated Connect Four game to a player's Elo.
